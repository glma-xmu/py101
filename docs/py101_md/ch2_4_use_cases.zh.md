...
出路是一个**装饰器（decorator）**：一个高阶函数，它接收一个函数，给它裹上一些额外行为，再把裹好的版本返回。它就是 2.3 §3.3 那个透传包装器，被赋予了一个名字和一个用途。
...
你其实已经写过一个正是这个形状的东西。2.1 §5 的牛顿法说的是：*改进猜测；如果已经足够好就停下；否则再做同样的事。* 那就是一个基准情形加一个递归情形，而我们当时用 `while` 循环来表达它，只因为手里没有别的工具。写成递归，描述和代码就变成了同一句话：

???+ example "示例：牛顿法，递归版"
    ```python
    def sqrt_newton(a, tolerance=1e-10, x=None):
        if x is None:                       # 第一次调用：挑一个起始猜测
            x = a / 2
        if abs(x * x - a) <= tolerance:     # 基准情形：够好了，停下
            return x
        better = x - (x * x - a) / (2 * x)  # 改进一次
        return sqrt_newton(a, tolerance, better)   # 再做同样的事

    print(sqrt_newton(2))       # 1.4142135623746899
    print(sqrt_newton(9))       # 3.0
    ```

把它和 2.1 §6 的循环版本对照：算术是一样的，变了的是**谁**记住当前的猜测。循环把它留在一个帧里，一遍遍重新绑定 `x`。递归让每一次改进都拥有**自己的帧**，猜测则作为实参一路往前传。也留意 2.1 §6 的 `None` 哨兵在这里真正派上了用场——起始猜测取决于 `a`，所以它没法写成一个固定的默认值。
